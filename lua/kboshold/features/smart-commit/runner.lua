...
function M.all_tasks_complete()
  for _, task in pairs(M.tasks) do
    if task.state == M.TASK_STATE.RUNNING or task.state == M.TASK_STATE.PENDING or task.state == M.TASK_STATE.WAITING then
      return false
    end
  end
  return true
      ...
      for id, task_state in pairs(M.tasks) do
        if task_state.state == M.TASK_STATE.WAITING then
          ...
          for _, dep_id in ipairs(task_state.depends_on) do
            if not M.tasks[dep_id] or 
               (M.tasks[dep_id].state ~= M.TASK_STATE.SUCCESS and 
                M.tasks[dep_id].state ~= M.TASK_STATE.SKIPPED) then
              can_run = false
              break
            end
          end
